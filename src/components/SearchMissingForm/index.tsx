...
const HELP_STEPS: HelpStep[] = [
  {
    n: 1,
    t: 'Subir Fotografía',
    d: 'Sube una foto del rostro de la persona que buscas. Se requiere visibilidad frontal clara.',
  },
  {
    n: 2,
    t: 'Cotejo Facial AI',
    d: 'El sistema extrae un vector facial (embedding) y lo compara por distancia coseno en ChromaDB.',
  },
  {
    n: 3,
    t: 'Ver Coincidencias',
    d: 'Los resultados se ordenan por semejanza. Verás la descripción física y el centro de refugio.',
  },
  {
    n: 4,
    t: 'Contacto Seguro',
    d: 'Ante una coincidencia cierta, solicita el reencuentro y preséntate con tu identificación oficial.',
  },
];
...
export default function SearchMissingForm() {
  ...
  const removePhoto = (idx: number) =>
    form.setFieldValue('photos', (prev: unknown) => toPhotos(prev).filter((_, i) => i !== idx));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 leading-snug">
              Sube una foto y busca coincidencias por reconocimiento facial.
            </p>
          </div>
        </div>
        <Button
      ...
      {!searchResults ? (
        ...
        >
                ...
                return (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {toPhotos(photos).length > 0 && (
                        <span className="text-[11px] font-semibold text-slate-400">
                          {toPhotos(photos).length}/{MAX_IMAGES}
                        </span>
                      )}
                    </div>
                    <PhotoUploader
            ...
            <div className="space-y-2 pt-1">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Completa su nombre o su cedula
                  <strong> (no hacen falta ambos).</strong>
                </p>
              </div>
                  ...
                  {(field) => (
                    <div className="space-y-1.5">
                      <label
                        ...
                        className="text-[11px] font-semibold text-slate-500 normal-case block"
                      >
                        Nombre
                      </label>
                      <input
          ...
          {isAnalyzing ? (
            ...
            <Button variant="rose" type="submit" className="w-full" disabled={isSubmitting} id="btn-trigger-search">
        ...
        <div className="space-y-6" id="search-results-section">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-rose-50/50 border border-rose-100 rounded-xl p-5">
            <div>
              ...
              <h3 className="text-sm font-semibold text-slate-800 mt-0.5">
                Se encontraron {searchResults.length} registros coincidentes.
              </h3>
            </div>
            <Button variant="rose" className="w-full sm:w-auto py-2.5" onClick={handleResetSearch} id="btn-re-search">
          ...
          <div className="space-y-3">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">
              Posibles coincidencias ÔÇö toca una para ver sus datos:
            </p>
            ...
            <div className="overflow-y-auto p-5 sm:p-6 space-y-4 text-sm" id="candidate-detail-pane">
              <div>
                ...
                <p className="text-slate-600 flex items-start gap-1.5">
                  ...
                  <span>
                    <strong>{selectedCandidate.hospitalName}</strong> - {selectedCandidate.locationAddress}
                  </span>
                </p>
              </div>
